...
## Issues and Resolution for changeOwner:

- https://github.com/uport-project/ethr-did-registry/issues/9

```js
describe('as new owner', () => {
        let tx
        before(async () => {
          const sig = await signData(signerAddress, signerAddress2, privateKey2, Buffer.from('changeOwner').toString('hex') + stripHexPrefix(badboy))
          tx = await didReg.changeOwnerSigned(signerAddress2, sig.v, sig.r, sig.s, badboy, {from: delegate4})
        })
        it('should change owner mapping', async () => {
          const owner2 = await didReg.owners(signerAddress)
          assert.equal(owner2, badboy)
        })
        ...
        it('should create DIDOwnerChanged event', () => {
          ...
          assert.equal(event.args.owner, badboy)
...
## Verifying Messages in Solidity:
...
<Summary>
Trouble with signature verification using `signer.signMessage` with `Web3Provider` #245
</Summary>
https://github.com/ethers-io/ethers.js/issues/245
